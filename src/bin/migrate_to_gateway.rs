...
//! source of truth — its existing rows are never overwritten. Genshin's
...
//!
//! Usage:
//!   DATABASE_URL=postgres://gr:password@localhost:5432/genshin_roles \
//!   GATEWAY_DATABASE_URL=postgres://app:password@localhost:5433/auth_gateway \
...
async fn main() {
    ...
    let genshin_url = env::var("DATABASE_URL")
        .expect("DATABASE_URL (Genshin plugin DB) must be set");
    let gateway_url = env::var("GATEWAY_DATABASE_URL")
        .expect("GATEWAY_DATABASE_URL (Auth Gateway DB) must be set");

    println!("Connecting to source (Genshin)...");
    let src = PgPoolOptions::new()
        .max_connections(4)
        .acquire_timeout(std::time::Duration::from_secs(10))
        .connect(&genshin_url)
        .await
        .expect("failed to connect to Genshin DB");
...
async fn migrate_discord_tokens(src: &PgPool, dst: &PgPool) {
    ...
    // a freshly-bootstrapped Genshin DB).
...
async fn migrate_user_guilds(src: &PgPool, dst: &PgPool) {
    ...
    // pre-date `guild_name` (mig 009) or `manage_guild` (mig 011).
