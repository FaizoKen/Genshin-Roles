...
<body>
    ...
    <div id="linked-section" class="card hidden">
        ...
        <p style="margin-top:14px; font-size:13px; color:#94a3b8;">
            Receiving Genshin roles in servers you didn't intend?
            <a href="/auth/my_servers?from=/genshin-player-role/verify" style="color:#74b9ff;">Choose which servers receive roles →</a>
        </p>
        <hr class="divider">
        <div class="actions">
    ...
    <script>
    const API = '{base_url}';
    const PLUGIN_SLUG = 'genshin-player-role';
    ...
    (function patchLoginHref() {{
        ...
        if (!link) return;
        const returnTo = '/genshin-player-role/verify?guild=' + encodeURIComponent(guildId);
        link.href = '/auth/login?return_to=' + encodeURIComponent(returnTo);
    }})();

    // Plugin-relative API helper (existing endpoints).
    ...
    async function applyGuildContext() {{
        ...
        if (wasDisabled) {{
            showGuildCtx(isLinked
                ? 'Enabled Genshin roles for ' + nameHtml + ' — roles apply on the next sync.'
                : 'Enabled Genshin roles for ' + nameHtml + ' — finish linking below to receive roles.');
        }} else {{
            showGuildCtx(isLinked
                ? 'Genshin roles are active in ' + nameHtml + '.'
                : 'Once linked, Genshin roles will apply in ' + nameHtml + '.');
        }}
    }}
        ...
        Ok(enka_result) => {
            ...
            let _ = sqlx::query(
                "INSERT INTO player_cache (uid, player_info, region, enka_ttl, next_fetch_at, \
                 level, world_level, achievements, abyss_progress, abyss_stars, fetter_count) \
                 VALUES ($1, $2, $3, $4, $5, \
                 ...
                 COALESCE(($2->>'towerStarIndex')::int, 0), \
                 ...
                 abyss_stars = COALESCE(($2->>'towerStarIndex')::int, 0), \
