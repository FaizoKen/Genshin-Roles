...
/// At 1M users with 50K active: active users refreshed every ~3.5 days,
/// inactive every ~7+ days (skipped until active threshold met).
...
pub async fn run(state: Arc<AppState>) {
    ...
    loop {
        match pick_next_user(&state).await {
            ...
            Ok(None) => {
                // No users need refreshing, sleep longer
